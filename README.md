...
This mod relies on two NVSE functions, `EnableFastTravel` and `ToggleDisableSaves`. Of these two, `EnableFastTravel` can be called from any script but `ToggleDisableSaves` *requires* an owning plugin in the load order and calling it from a script runner will not actually toggle the ability to save. For the save toggle to work, this mod requires a helper function hosted by a proper ESP plugin, as follows:
...
This function *must* be embedded in an ESP plugin for the mod to work. It can be merged into any already present plugin to save a plugin slot. Alternatively, you can use the included `ESM` plugin directly, if you've got slots to spare.
